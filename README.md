...
The model trained with planet mass achieved a Mean Absolute Error (MAE) of 1.11 Earth radii and an R² score of 0.763. The model trained without planet mass had a higher MAE of 1.88 Earth radii and a lower R² score of 0.542.

| Model | MAE | R² |
| With Planet Mass | 1.11 Earth radii | 0.763 |
| Without Planet Mass | 1.88 Earth radii | 0.542 |
 
This suggests that planet mass provides significant predictive information for estimating planet radius in this dataset.

## Limitations

Missing data was a major limitation in training the model. More complete data could've helped the model predict the radius more accurately. The model was trained on 80% and only tested on 20% of the data; a different split could've resulted in different values. The main purpose of the project was to build a deeper understanding of machine learning and experiment with it in a real-world case, not for scientific evidence.
...
Then run the Python script. The exoplanet data is retrieved directly from the NASA Exoplanet Archive when the program runs, so no separate dataset download is required.
